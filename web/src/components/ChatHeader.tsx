    ...
    chatId: string }) 
{
  ...
  const isOnline = Boolean(participant?._id && onlineUsers.has(participant._id));
  ...
  return (
    <div className="h-16 px-6 border-b border-base-300 flex items-center gap-4 bg-base-200/80">
      <div className="relative">
        <img src={participant?.avatar} alt={participant?.name ?? "avatar"} className="w-10 h-10 rounded-full bg-base-300/40" />
        {isOnline && (
